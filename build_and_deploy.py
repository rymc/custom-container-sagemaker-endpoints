...
if len(sys.argv) < 2:
    print("Usage: python script.py <image-name>")
    sys.exit(1)
...
run_command(f"aws ecr get-login-password --region {region}")
...
artifact = wandb.Artifact(name="Dockerfile", type="dockerfile")
artifact.add_file("Dockerfile")  # Add dataset directory to artifact
